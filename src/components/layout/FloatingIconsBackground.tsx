...
interface BackgroundIconConfig {
  ...
  wrapperClassName: string; // For positioning, opacity
  iconClassName: string; // For sizing the Lottie player
  animationClassName: string; // For custom animation timing (e.g., "[animation-duration:18s] [animation-delay:-2s]")
}
      ...
      {backgroundIcons.map(({ id, lottieSrc, wrapperClassName, iconClassName, animationClassName }) => (
        <div
          ...
          className={cn(
            ...
            animationClassName // Handles custom animation duration and delay via Tailwind arbitrary classes
          )}
        >
          <Player
            autoplay
